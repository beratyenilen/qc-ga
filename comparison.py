...
from tools import load, load_state, plotCircLengths
...
#    perm_desired_state = desired_state
    ...
    dens_matr = result.data(
    )['snapshots']['density_matrix']['final'][0]['value']
    ...
    # How virtual bits map to physical bits
    qubit_pattern = list(circ._layout.get_virtual_bits().values())
...
#    circ.remove_final_measurements()
    ...
    perm_circ = Permutation(n_phys, qubit_pattern)
    ...
    qiskit_circs, depths = genCircs(
        numberOfQubits, fake_machine, desired_state, n_iter=10)
    ...
    fidelities = getFidelities(
        5, qiskit_circs, machine_simulator, fake_machine, desired_state)
    # fidelities.append(s/100)
    plt.hist(fidelities, bins=list(np.arange(0, 1.2, 0.01)),
             align='left', color='#AC557C')
    print('!!!')
...
def main():
    numberOfQubits = 3
    desired_state = load_state(numberOfQubits, 42)
